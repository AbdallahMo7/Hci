...
if __name__ == "__main__":
    ...
    # Initialize Gaze_Tracking with the default webcam
    gaze_tracker = Gaze_Tracking(Path="")  # Empty Path means webcam is used

    try:
        print("Starting gaze tracking using webcam. Press 'Esc' to exit.")

        while gaze_tracker.Capture.isOpened():
            success, frame = gaze_tracker.Capture.read()
            if not success:
                print("Unable to capture frame from the webcam.")
                break

            # Process the frame
            gaze_tracker.Process_Frames(frame)

            # Draw the detected iris positions
            frame_with_iris = gaze_tracker.Draw_Iris()

            # Display the frame
            cv2.imshow("Gaze Tracking", frame_with_iris)

            # Break the loop if 'Esc' key is pressed
            if cv2.waitKey(1) & 0xFF == 27:  # Esc key to exit
                print("Exiting gaze tracking.")
                break
    except Exception as e:
        print(f"An error occurred: {e}", file=sys.stderr)
    finally:
        # Release resources
        gaze_tracker.Capture.release()
        cv2.destroyAllWindows()

        # Generate heatmap
        print("Generating heatmap...")
        gaze_tracker.Generate_Heatmap()
